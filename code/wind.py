...
device = 0 if torch.cuda.is_available() else (0 if torch.has_mps else -1)  # Use GPU if available, MPS for Apple Silicon, else CPU
...
def update_summary_text(chunk_summary):
    ...
    window.summary_text_edit.append(chunk_summary)  # Append new chunk
